...
# docker run --platform linux/amd64 -dit --rm -v /tmp/artifacts:$(pwd)/artifacts --mount type=tmpfs,destination=/build,tmpfs-mode=1770,tmpfs-size=53687091200 build-direct
# docker run --platform linux/amd64 --rm -v /tmp/artifacts:$(pwd)/artifacts --tmpfs /build:exec,size=53687091200 build-direct
FROM ubuntu:bionic

RUN apt update
RUN apt install -y git curl lsb-release sudo apt-utils
# speed up the process of build-essentials by caching the current dependencies
